...
extern bool isValidArgs(uint32_t& commandIndex, std::vector<std::string>& inputArgs);
extern bool isValidPath(std::string& path);
extern void lsExec();
extern void mkdirExec(std::istringstream& argsStringStream);
extern void mvExec();
extern void rmExec();
extern void setArgVec(uint32_t commandIndex, std::string& argsString, std::vector<std::string>& inputArgs);
